...
export async function POST(_request: Request, context: { params: Promise<{ ruleId: string }> }) {
  ...
  if (!usesLegacyOperations()) return proxyOperations(new Request(_request.url, { method: "POST" }), `/operations/rules/${encodeURIComponent(ruleId)}/enable`);
  try {
    const rule = await setRuleEnabled(ruleId, true);
