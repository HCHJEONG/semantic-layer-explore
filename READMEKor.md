# Semantic Layer Explorer (한국어)

물리적 워크스페이스를 위한 AI 기반 의미론적 레이어 탐색기입니다. 온톨로지 기반 지식 그래프, 실시간 센서/기기 제어, Gemini AI를 활용한 자연어 질의, 규칙 기반 자동화를 통합한 풀스택 애플리케이션입니다.
...
#### A. 온톨로지 기반 탐색기 (Ontology Explorer)
...
#### B. 실시간 대시보드 (Workspace Dashboard)
...
- **Gemini Function Calling** — 6개 도구를 사용한 단계적 데이터 수집
- **도구 목록**:
...
#### D. Rule Studio (자동화 규칙 생성)

- **자연어 → 규칙 변환**: "온도가 30도를 넘으면 팬을 켜" → 구조화된 규칙 JSON
- **Gemini 6단계 Tool Calling**: 온톨로지 → 센서 → 기기 → 규칙 확인
...
| **AI/LLM** | Google Gemini 2.0 Flash | 자연어 처리, Function Calling |
...
| 구분 | 자동 제어 (Rule Engine) | Ask AI (Gemini) |
|---|---|---|
| **트리거** | 센서 이벤트 발생 | 사용자 질문 입력 |
| **처리 방식** | `rule-engine.ts` (로컬 평가) | Gemini Function Calling (6단계) |
| **속도** | ~10-50ms | ~2-5초 |
| **비용** | 무료 (로컬) | Gemini API 호출 비용 |
| **신뢰성** | 항상 동작 (오프라인 가능) | API 의존 |
| **용도** | "IF 온도 > 30 THEN 팬 ON" | "왜 팬이 켜졌어?" |
...
```
1단계: 자연어 → 규칙 JSON (Gemini, 6단계 tool calling)
  ↓ (사용자 검토/승인)
...
3. **2단계 파이프라인** — 생성(Gemini)과 실행(로컬) 분리로 안전성과 성능 확보
